...
void setup()
{
  size(1024, 768, P2D);
  reset();
  img01 = loadImage("assets/data/albums/01.jpg");
...
void draw()
{
  ...
  for (Triangle t : myTriangles)
  {
    ...
    //fill(225, map(t.p1.x, 0, height, 0, 225), map(t.p1.y, 0, width, 0, 225));
    noFill();
    //fill(map(t.p1.x, 0, height, 0, 225), 100, map(t.p1.y, 0, width, 0, 225));
    vertex(t.p1.x, t.p1.y);
    vertex(t.p2.x, t.p2.y);
    vertex(t.p3.x, t.p3.y);
  }
  endShape();
  int total = myTriangles.size();
  ...
  for (Triangle t : myTriangles)
  {
    ...
    {
      text("Juju On That Beat (TZ Anthem)", t.p3.x, t.p3.y);
    }
    else if(total ==33)
    {
    ...
    {
      text("Heathens, twenty one pilots", t.p3.x, t.p3.y);
    }
    else if(total ==39)
      text("Let Me Love You, DJ Snake Featuring Justin Bieber‬‬", t.p3.x, t.p3.y);

    else if(total ==42)
      text("Broccoli‬,  D.R.A.M. Featuring Lil Yachty‬", t.p3.x, t.p3.y);

    else if (total ==45)
      text("Don't Wanna Know, Maroon 5 Featuring Kendrick Lamar", t.p3.x, t.p3.y);

    else if(total ==48)
      text("Fake Love‬, Drake", t.p3.x, t.p3.y);

    else if(total ==52)
      text("Caroline‬, ‪Amine", t.p3.x, t.p3.y);

    else if(total ==52)
      text("Starving, Hailee Steinfeld & Grey Featuring Zedd", t.p3.x, t.p3.y);

    else if(total ==13)
      text("I Hate U I Love U, gnash Featuring Olivia O'Brien", t.p3.x, t.p3.y);

    else if(total ==16)
      text("Cold Water, Major Lazer Featuring Justin Bieber & MO", t.p3.x, t.p3.y);

    else if(total ==19)
      text("QTreat You Better, Shawn Mendes", t.p3.x, t.p3.y);

    else if(total ==50)
      text("The Greatest, Sia Featuring Kendrick Lamar", t.p3.x, t.p3.y);
...
class MyPoint
{
  final static float MAX_DIST_MOUSE = 40;//60//mouse influence zone
  final static float MAX_DIST_POINTS = 45;//45;//other points influence zone
